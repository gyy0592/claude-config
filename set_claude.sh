...
# v2.5: REFLECT-state rebuttal nag — fires every PreToolUse while
...
# v2.6: hard FSM-completion audit. ALWAYS ON. No yaml toggle. Blocks stop
# until stage_history (since last RESET_TO_BOOT) contains every required
# spine event AND current_status==END. Hostile rant on miss.
STOP_STATE_AUDIT_SCRIPT = os.path.expanduser("~/.claude/hooks/stop_state_audit.sh")
ensure_hook("Stop", None, f"bash {STOP_STATE_AUDIT_SCRIPT}")
