[🇺🇸 English](ten_commandments_for_ai_coding.md)

# AI 辅助编程十戒

**第零戒（最高优先级）：任何任务或症状，必须先经过「测量 → 假设 → 排查」中间层，严禁直接行动**

* **细则**：
  1. **先测量，不急于行动**：遇到任何问题/目标/结果不符预期，第一步是识别并列出所有可观测指标，用工具获取实际数据，写入文件（如 `metrics2monitor.md`）。严禁跳过测量直接修改代码/配置。
  2. **从数据推出假设空间（必须多个）**：每个测量结果必须对应多个可能原因，写成假设列表。严禁从观测值直接跳到单一结论。
  3. **对每个假设独立验证并结构化记录**：逐条检验，每条写出「证据 + 结论（确认/排除）」，写入 `monitor_spec.md` 或等效文件。每项检查对象必须独立出现；严禁整体性结论（「总体没问题」「感觉正常」）。
  4. **才执行行动**：排查出根因后，再针对性修复。

* **原理**：AI 只有文字拟合能力，训练数据里「代码慢 → 看代码 → 改代码」的模式占主导。而「先量系统指标 → 分叉多个假设 → 逐条排查」这一中间层，是领域专家默认执行但从不显式写出的隐性知识——AI 从训练数据里拟合不到它。不强制走这一层，AI 会直奔最显眼的表层动作，在不知真正根因的情况下盲目修改，改完还声称「已解决」。强制结构化输出（每个检查项必须有独立条目 + 证据 + 结论写入文件）的根本作用：AI 无法对尚未写出的条目声称「通过」，从而杜绝「感觉达标」式的偷懒。

* **正例（GPU 训练代码跑很久）**：
  1. 先量：运行 `nvidia-smi` → 记录每张卡利用率、显存；写入 `metrics2monitor.md`
  2. 利用率 30%（低）→ 立刻分叉假设：A) DataLoader 单进程瓶颈？B) torchrun 并行未用满？C) 多卡通信开销？D) CPU 绑定？
  3. 逐条检验 → 查 `num_workers`（=0）→ 查 `htop`（CPU 满载）→ 确认根因：DataLoader 单进程
  4. 对每个假设写 pass/fail + 理由 → 写入 `monitor_spec.md`
  5. 修复：`num_workers=8`，重跑 → 利用率升至 85% → 记录验证结果

* **反例（AI 无头苍蝇式）**：
  1. 代码跑很久 → 直接打开训练脚本
  2. 扫一遍：「这个 for 循环可能低效」→ 改成向量化
  3. 报告：「已优化代码，应该会快一些」
  4. 实际：瓶颈在 DataLoader；代码改动毫无意义；利用率仍 30%；根因未查
...
*改编自 Dr. Sihao Liu 的 [Humanize](https://github.com/humania-org/humanize) 方法论，并加入个人修改与补充。*
