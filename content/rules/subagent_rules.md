...
F. **Rebuttal protocol** — see `reflect_protocol.md` + `reflect_subagent_prompt.md`. Quick form: write your draft under `## reviewer reply` in `reflection_<round_id>.md`, sleep `while true; do sleep 30; done`, on each SendMessage append `### round N`, exit on `[CONSENSUS_REACHED]` or N rounds.
