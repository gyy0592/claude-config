...
- **第 2 步：动手 + 周期监控**。倍增间隔（120 → 240 → 480 → 600 秒封顶）监控。每次对清单每条跑测量命令，写 [观察]（真实命令真实输出 + 数值 + 判断结论 + 严重程度 + 反证审查）。
...
- 启动后必须按倍增间隔周期监控：第 1 次启动后 120 秒查；第 2 次再过 240 秒查；第 3 次再过 480 秒查；之后封顶 600 秒（10 分钟）。**频率不能更低 — 间隔超过 600 秒 = 失职。**
...
### [反思] 条目（4 步 workflow 第 1/3/4 步必用）

```
[反思 主题] YYYY-MM-DD HH:MM UTC | 一轮自问：<具体问题> → 一轮自答：<具体答案> | 二轮自问：<挑战上一轮答案> → 二轮自答：<答案> | （可继续 N 轮直到无疑虑） | 结论：无疑虑 / 仍有疑虑 → 触发动作：<下一步>
```

第二轮必须挑战第一轮的答案；仅写一行「我反思了」或两行复读 = 谎报军情。
